...
const sources = (id: string, modelType: ModelType): Record<ExampleTab, string> => {
...
export async function modelExamples(id: string, modelType: ModelType): Promise<CodeExamples> {
  const source = sources(id, modelType);
