...
| Regenerate PostgreSQL row types | `bun run db:types` (`--verify` checks they are current) |
...
- `src/db-types.ts` — `Tables`, the row type of every PostgreSQL table as the
  `postgres` driver returns it, generated by `bun run db:types`. Never edit
  it by hand; derive query row types from it with `Pick<Tables["…"], …>`.
...
7. **Schema changes are new numbered files** in `migrations/<store>/`,
   ...
   After adding a PostgreSQL migration, run `bun run db:types` and commit
   the regenerated `src/db-types.ts`.
